...
export function Sidebar() {
    ...
    return (
        <>
            {/*
              * El aside ocupa toda la altura via h-screen del padre (Layout).
              * flex-col + overflow-hidden garantiza que nada se desborde.
              * La sección del medio (nav + invoice card) usa flex-1 + overflow-y-auto
              * para ser scrollable, mientras que el footer de botones queda
              * siempre visible sin importar la altura de la pantalla.
            */}
            <aside className="w-64 bg-[var(--sidebar-bg)] border-r border-slate-200 dark:border-slate-800 flex flex-col shadow-xl z-20 transition-colors duration-300 overflow-hidden">

                {/* ── Logo Header (estático, nunca se oculta) ── */}
                <div className="flex-shrink-0 p-6 flex items-center space-x-3 border-b border-slate-100 dark:border-slate-800">
                ...
                {/* ── Zona scrollable: navegación + Invoice Card ── */}
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    {/* Invoice Card – scrollable con el nav */}
                    <div className="px-4 pb-4">
                        <div className="relative bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-[4px_4px_0px_0px_rgba(203,213,225,1)] dark:shadow-[4px_4px_0px_0px_rgba(2,6,23,1)] transform transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_0px_rgba(6,182,212,0.3)]">
                            <Link
                                to="/billing"
                ...
                {/* ── Footer fijo: Configuración + Cerrar Sesión + Brand ── */}
                {/* flex-shrink-0 impide que este bloque se comprima o desaparezca */}
                <div className="flex-shrink-0 p-4 border-t border-slate-100 dark:border-slate-800 space-y-1">
                    <button
                        onClick={() => setIsSettingsOpen(true)}
                        className="w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm text-slate-500 dark:text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors"
                    >
                    ...
                    <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex justify-center">
                        <a href="https://neuracall.net/" target="_blank" rel="noopener noreferrer" className="opacity-70 hover:opacity-100 transition-opacity">
